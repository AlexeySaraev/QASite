...
type ModelType = "gemini" | "groq" | "cerebras";
type TaskType = "requirements" | "testcases" | "code";

export default function Home() {
  const [input, setInput] = useState("");
  const [result, setResult] = useState("");
  const [model, setModel] = useState<ModelType>("gemini");
  const [taskType, setTaskType] = useState<TaskType>("requirements");
    ...
    <main className="max-w-5xl mx-auto px-4 py-12 sm:py-20">
      ...
      <div className="text-center mb-12">
        ...
        <p className="text-lg text-slate-500 max-w-2xl mx-auto">
          Умный помощник для анализа требований, генерации тест-кейсов и проверки кода.
        </p>
      </div>

      {/* Панель выбора модели и типа задачи */}
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className={`flex-1 py-2.5 px-3 rounded-xl text-sm font-medium transition-all border ${
                  model === "groq"
                    ? "bg-blue-50 border-blue-500 text-blue-700 shadow-sm"
                    : "bg-slate-50 border-slate-200 text-сlate-600 hover:bg-сlate-100"
                }`}
              >
                ⚡ Groq (Llama 3)
              </button>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 mb-6">
        <textarea
          placeholder="Вставьте требования, описание функционала или код..."
